...
FED_SCORE_NORMALIZATION=${FED_SCORE_NORMALIZATION:-"hybrid"}
FED_SCORE_EMA_BETA=${FED_SCORE_EMA_BETA:-"0.90"}
FED_SCORE_MIN_STD=${FED_SCORE_MIN_STD:-"1.0"}
FED_SCORE_HYBRID_RAW_WEIGHT=${FED_SCORE_HYBRID_RAW_WEIGHT:-"0.35"}
FED_SCORE_HYBRID_SCALE=${FED_SCORE_HYBRID_SCALE:-"50"}
  ...
  for SEED in $SEEDS; do
    for VARIANT in $FED_VARIANTS; do
      ...
      python3 -m src.main \
        ...
        --fed-score-hybrid-raw-weight "$FED_SCORE_HYBRID_RAW_WEIGHT" \
        --fed-score-hybrid-scale "$FED_SCORE_HYBRID_SCALE" \
